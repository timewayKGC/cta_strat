"""
This is a demo for simple open range intraday trading strat
range = high[-1] - low[-1]
if price breaks open + range, long
if price drops to open - range, short
clear at close
"""
...
from OpenRangeSignal import OPSignal

########################################################################
# 策略继承CtaTemplate
class OpenRange(CtaTemplate):
    ...
    def __init__(self, ctaEngine, setting):
        # 首先找到策略的父类（就是类CtaTemplate），然后把OPStrategy的对象转换为类CtaTemplate的对象
    ...
    def strategy(self, bar):
        band_Period= self.timeframeMap["bandPeriod"] # for band calculation
        signal_Period= self.timeframeMap["signalPeriod"]# for break band calculation
        # 根据出场信号出场
        exitSig = self.exitSignal(band_Period)#only determined by band period's closing price
        print('exitSig:', exitSig)
        self.exitOrder(bar, exitSig)

        # 根据进场信号进场
        entrySig = self.entrySignal(band_Period, signal_Period)# determined by band and break band signals
    ...
    def exitSignal(self, band_Period):
        arrayPrepared1, amBand = self.arrayPrepared(band_Period)
        timeSignal = 0
        if arrayPrepared1:
            if amBand.datetime[-1] == "15:00:00":#?? don't know if amBand has time attribute
                timeSignal = 1
        return timeSignal

    def exitOrder(self, bar, exitSig): #what is bar?
        if self.posDict[self.symbol+'_LONG']>0:
            if exitSig==1:
                self.cancelAll()
                self.sell(self.symbol, bar.close*0.99, self.posDict[self.symbol+'_LONG'])
    ...
    def entrySignal(self, band_Period, signal_Period):
        arrayPrepared1, amBand = self.arrayPrepared(band_Period)
        arrayPrepared2, amSignal = self.arrayPrepared(signal_Period)
        entrySignal = 0
        if arrayPrepared1 and arrayPrepared2:
            algorithm = OPSignal()
            upper, lower = algorithm.BandCalc(amBand)
            break_Band= algorithm.breakBand(amSignal, upper, lower)
